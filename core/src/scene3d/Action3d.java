...
abstract public class Action3d implements Poolable {
         ...
         * called with null when an action is removed from an actor. When set to null, if the action has a {@link #setPool(com.badlogic.gdx.utils.Pool) pool}
         * then the action is {@link com.badlogic.gdx.utils.Pool#free(Object) returned} to the pool (which calls {@link #reset()}) and the pool is set to null.
